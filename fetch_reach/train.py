...
def evaluate(env: gym.Env, agent: SACAgent, seed: int, episodes: int = 20) -> tuple[float, float]:
    """Run the deterministic policy on a fixed set of goals (re-seeded each call),
    so successive checkpoints are compared on the same task. Returns (mean return,
    success rate)."""
    env.reset(seed=seed)
    returns, successes = [], []
...
def main():
    ...
    try:
        ...
        log_file.close()
    print(f"done in {time.time() - t0:.0f}s, best eval success {best[0]:.0%}, saved to {run_dir}/")
